          ...
          <div style={{ borderRadius:8, border:'1px solid #e0e0e0', overflow:'hidden' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
              <tbody>
                ...
                {/* Negative raw balance = driver paid ahead of earnings -> DRIVER ADVANCE
                    to recover next settlement (standard trucking term), not money owed to
                    the driver. Keeps this headline consistent with the "ahead of earnings"
                    row in the FIFO section below. */}
                <tr style={{background:'#1a2a3a'}}>
                  <td style={{ padding:'14px 12px', fontSize:15, fontWeight:900, color:'#fff', fontFamily:'var(--font-head)', letterSpacing:'0.04em' }}>{d.stillOwedRaw < 0 ? 'DRIVER ADVANCE — RECOVER NEXT SETTLEMENT' : 'BALANCE CURRENTLY OWED TO DRIVER'}</td>
                  <td style={{ padding:'14px 12px', textAlign:'right', fontSize:20, fontWeight:900, color: d.stillOwedRaw < 0 ? '#ffab40' : '#ffd54f', fontFamily:'var(--font-head)' }}>{fmt(Math.abs(d.stillOwedRaw))}</td>
                </tr>
...
export default function SettlementReport({ driverName, loads, showToast, ownerCutPct = 10 }) {
  ...
  function driverStats(dn) {
    ...
    return {
      ...
      stillOwed: rb.stillOwed,     // running balance — all-time correct answer (clamped >= 0)
      stillOwedRaw: rb.stillOwedRaw, // signed running balance: negative = driver advanced ahead of earnings
    }
  }
        ...
        <div>
            ...
            return (
              <div key={dn} className="card" style={{ borderLeft:'3px solid ' + color, marginBottom:12 }}>
                ...
                <div style={{ borderTop:'2px solid var(--border)', marginTop:10, paddingTop:10 }}>
                  ...
                  {/* Signed running balance. Negative raw = driver was paid ahead of
                      earnings, so the standard trucking term is a DRIVER ADVANCE to be
                      recovered next settlement — not money owed to the driver. Positive
                      (or zero) = Balance Owed to Driver. White-label: the label uses the
                      accounting term, never a driver name. */}
                  <div className="amount-row">
                    <span className="label" style={{fontWeight:900,color:'var(--white)',fontSize:14}}>
                      {s.stillOwedRaw < 0 ? 'Driver Advance (recover next settlement)' : 'Balance Owed to Driver'}
                    </span>
                    <span className="value" style={{color: s.stillOwedRaw < 0 ? '#e65100' : 'var(--amber)', fontSize:18, fontWeight:900}}>{fmt(Math.abs(s.stillOwedRaw))}</span>
                  </div>
                </div>
          ...
          <button
            onClick={() => {
              ...
              // Default the payee to a driver other than the logged-in owner. An owner
              // recording a disbursement is almost always paying a driver, not himself;
              // pre-loading his own name is what mislabeled TIM's payments as BRUCE.
              // Bookkeeper (driverName === null) keeps the existing TIM default.
              if (!isBookkeeper) {
                const other = driverNames.find(d => d !== driverName)
                setPayDriver(other || driverName)
              }
            }}
          ...
          {showPayDrawer && (
            <div className="card" style={{ marginBottom:12, border:'1px solid #2a4a2a' }}>
              ...
              {/* Driver picker shows whenever there is more than one driver to choose
                  from — bookkeeper OR owner. Prior bug: gated on isBookkeeper only, so
                  an owner (Bruce) saw no picker and every payment saved under the
                  logged-in owner instead of the intended payee. */}
              {(isBookkeeper || driverNames.length > 1) && (
                <div style={{ marginBottom:12 }}>
                  <div style={{ fontSize:11, color:'var(--grey)', fontFamily:'var(--font-head)', marginBottom:6 }}>DRIVER</div>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                    {driverNames.map(d => (
